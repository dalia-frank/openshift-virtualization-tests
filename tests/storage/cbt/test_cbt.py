...
class TestFullBackupRestore:
    ...
    def test_full_backup_push_mode_restore(
        self,
        test_data_from_restored_vm_push_mode,
    ):
        """
        ...
        """
        assert test_data_from_restored_vm_push_mode == CBT_TEST_DATA

    @pytest.mark.polarion("CNV-15996")
    def test_full_backup_pull_mode_restore(
        self,
        test_data_from_restored_vm_pull_mode,
    ):
        """
        ...
        Expected:
            - Restored VM boots successfully and test data is present
        """
        assert test_data_from_restored_vm_pull_mode == CBT_TEST_DATA
...
class TestIncrementalBackupRestore:
    ...
    def test_incremental_backup_push_mode_restore(
        ...
        assert (
            read_file_content_from_vm(
                vm=restored_vm_from_incremental_backup_push_mode,
                file_path=CBT_INCREMENTAL_TEST_DATA_FILE,
            )
            == CBT_INCREMENTAL_TEST_DATA
        )
...
class TestMultipleIncrementalBackups:
    ...
    __test__ = False  # Exclude from PR1 - reserved for future PR
    ...
    def test_multiple_incremental_backups_push_mode_restore(
        ...
        Expected:
            - Restored VM boots successfully and all test data is present
        """
        restored_vm = restored_vm_from_second_incremental_backup_push_mode
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_MULTI_INCREMENTAL_TEST_DATA_FILE_PHASE_1)
            == CBT_MULTI_INCREMENTAL_DATA_PHASE_1
        )
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_MULTI_INCREMENTAL_TEST_DATA_FILE_PHASE_2)
            == CBT_MULTI_INCREMENTAL_DATA_PHASE_2
        )
    ...
    def test_multiple_incremental_backups_pull_mode_restore(
        ...
        Expected:
            - Restored VM boots successfully and all test data is present
        """
        restored_vm = restored_vm_from_second_incremental_backup_pull_mode
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_MULTI_INCREMENTAL_TEST_DATA_FILE_PHASE_1)
            == CBT_MULTI_INCREMENTAL_DATA_PHASE_1
...
class TestMultipleDiskBackup:
    ...
    def test_backup_multiple_disks_push_mode_restore(
        ...
        """
        restored_vm = restored_vm_from_multi_disk_backup_push_mode
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_DATA_DISK_TEST_DATA_FILE) == CBT_DATA_DISK_TEST_DATA
        )
    ...
    def test_backup_multiple_disks_pull_mode_restore(
        ...
        Expected:
            - Restored VM boots successfully and test data from both disks is present
        """
        restored_vm = restored_vm_from_multi_disk_backup_pull_mode
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_DATA_DISK_TEST_DATA_FILE) == CBT_DATA_DISK_TEST_DATA
        )
...
class TestBackupAfterLiveMigration:
    ...
    def test_incremental_backup_after_live_migration_push_mode(
        ...
        """
        restored_vm = restored_vm_after_migration_incremental_push
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_POST_MIGRATION_TEST_DATA_FILE)
            == CBT_POST_MIGRATION_TEST_DATA
        )
    ...
    def test_incremental_backup_after_live_migration_pull_mode(
        ...
        Expected:
            - Restored VM boots successfully and pre-migration and post-migration test data are present
        """
        restored_vm = restored_vm_after_migration_incremental_pull
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_POST_MIGRATION_TEST_DATA_FILE)
            == CBT_POST_MIGRATION_TEST_DATA
...
class TestHotplugBackup:
    ...
    def test_backup_with_hotplugged_disk_push_mode_restore(
        ...
        """
        restored_vm = restored_vm_hotplug_push
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_HOTPLUG_DISK_TEST_DATA_FILE)
            == CBT_HOTPLUG_DISK_TEST_DATA
        )
    ...
    def test_backup_with_hotplugged_disk_pull_mode_restore(
        ...
        Expected:
            - Restored VM boots successfully and test data from both original and hotplugged disks is present
        """
        restored_vm = restored_vm_hotplug_pull
        assert read_file_content_from_vm(vm=restored_vm, file_path=CBT_BOOT_DISK_TEST_DATA_FILE) == CBT_TEST_DATA
        assert (
            read_file_content_from_vm(vm=restored_vm, file_path=CBT_HOTPLUG_DISK_TEST_DATA_FILE)
            == CBT_HOTPLUG_DISK_TEST_DATA
...
class TestBackupErrorHandling:
    ...
    def test_backup_fails_when_storage_full_push_mode(
        self,
        vm_still_accessible_after_failed_push_backup,
    ):
        """
        ...
        """
        assert vm_still_accessible_after_failed_push_backup == CBT_TEST_DATA
    ...
    def test_backup_fails_when_storage_full_pull_mode(
        self,
        vm_still_accessible_after_failed_pull_backup,
    ):
        """
        ...
        """
        assert vm_still_accessible_after_failed_pull_backup == CBT_TEST_DATA
...
class TestConcurrentBackups:
    ...
    def test_concurrent_backups_push_mode_restore(
        self,
        test_data_from_five_restored_vms_push_mode,
    ):
        """
        ...
        """
        assert all(test_data == CBT_TEST_DATA for test_data in test_data_from_five_restored_vms_push_mode)

    @pytest.mark.polarion("CNV-16012")
    def test_concurrent_backups_pull_mode_restore(
        self,
        test_data_from_five_restored_vms_pull_mode,
    ):
        """
        ...
        Expected:
            - All restored VMs boot successfully and test data is present in each VM
        """
        assert all(test_data == CBT_TEST_DATA for test_data in test_data_from_five_restored_vms_pull_mode)
...
class TestWindowsVMFullBackup:
    ...
    def test_windows_vm_full_backup_push_mode_restore(
        self,
        windows_test_data_from_restored_vm_push_mode,
    ):
        """
        ...
        """
        assert windows_test_data_from_restored_vm_push_mode == CBT_WINDOWS_TEST_DATA

    @pytest.mark.polarion("CNV-16014")
    def test_windows_vm_full_backup_pull_mode_restore(
        self,
        windows_test_data_from_restored_vm_pull_mode,
    ):
        """
        ...
        Expected:
            - Restored Windows VM boots successfully and test data is present
        """
        assert windows_test_data_from_restored_vm_pull_mode == CBT_WINDOWS_TEST_DATA
...
class TestWindowsVMIncrementalBackup:
    ...
    def test_windows_vm_incremental_backup_push_mode_restore(
        ...
        """
        restored_vm = restored_vm_from_incremental_backup_windows_push_mode
        verify_file_in_windows_vm(
            windows_vm=restored_vm,
    ...
    def test_windows_vm_incremental_backup_pull_mode_restore(
        ...
        Expected:
            - Restored Windows VM boots successfully and all test data is present
        """
        restored_vm = restored_vm_from_incremental_backup_windows_pull_mode
        verify_file_in_windows_vm(
            windows_vm=restored_vm,
            file_name_with_path=CBT_WINDOWS_TEST_DATA_FILE,
            file_content=CBT_WINDOWS_TEST_DATA,
